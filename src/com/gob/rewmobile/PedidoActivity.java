...
public class PedidoActivity extends FragmentActivity implements
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mDrawerList.setAdapter(new BloqueAdapter(this, Data.LST_CATEGORIAS,
				BloqueAdapter.ITEM_CATEGORIA));
	...
	private void enviar() {
		Log.i("ENVIAR", "Enviando");
		PrintTicket printTicket = new PrintTicket(this, PrintTicket.TMU220, PrintTicket.ENVIO, "192.168.1.20");
		printTicket.printDoc();
	}
	...
	private void updateLstProductos(final String co_categoria) {
		...
		if (db != null) {
			...
			if (co_categoria.isEmpty()) {
				fila = db.rawQuery(
						"SELECT id, no_producto, va_producto FROM productos",
						null);
			} else {
			...
			db.close();
			gridView.setAdapter(new BloqueAdapter(getBaseContext(),
					Data.LST_PRODUCTOS, BloqueAdapter.ITEM_PRODUCTO));
		}
	}
